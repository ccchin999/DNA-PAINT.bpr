...
class ReconsDataset(torch.utils.data.Dataset):#就是读图形成sampe，这部分就是准备数据集。把一个.npy文件中第一层取出来作为金标准，其他作为input，然后形成一个sample作为网络的input
     ...
     def __getitem__(self, idx):#idx相当于i用于循环。。。 此循环中sample个数一共1135个 
         ...
         IN[:,:,0] = io.imread(os.path.join(self.train_data_path, self.dirs_data[idx], "wide-field.tif")) # widefield.tif
         IN[:,:,1] = io.imread(os.path.join(self.train_data_path, self.dirs_data[idx], "sparsely-localized.tif"))/65535 # sparsely-localized.tif
         GT =  io.imread(os.path.join(self.train_data_path, self.dirs_data[idx], "ground-truth.tif"))/65535
...
# 调用显卡 “0”
    ...
    for epoch in range(1,(epoch_num+1)):#epoch：对所有的训练数据进行一次完整的训练，称为 一轮训练
        ...
        if epoch%10==0:#判断epoch，如果能被一百整除就保存这一轮的模型，
